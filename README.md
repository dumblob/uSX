...
The uSX **data model** is an unbounded list of records, where each record is a pair of an ID (a key) and a value. Because it's a list, order of the pairs matters and a key can be used more than once in the list (each time with an arbitrary value).
